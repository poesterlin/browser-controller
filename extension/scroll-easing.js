...
// per-frame scroll distances: the scroll starts at 70% of base speed and
// eases smoothly up to a hard cap of 130% of base speed. The single symmetric
// ramp averages exactly 1.0, so "base speed" is the true average and the cap
// is a real +30%.
...
export function easedScrollPositions(total, frameCount) {
  ...
  const deltas = [];
  for (let index = 0; index < frameCount; index += 1)
    deltas.push(START_FACTOR + (CAP_FACTOR - START_FACTOR) * smoothstep((index + 0.5) / frameCount));
  const positions = [0];
  let traveled = 0;
